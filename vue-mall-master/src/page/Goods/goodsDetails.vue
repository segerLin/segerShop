  ...
  <div class="w store-content">
    <div class="gray-box">
      ...
      <div class="banner">
        ...
        <div class="buy">
          <y-button text="加入购物车"
                    @btnClick="addCart(product.id,product.price,product.name,product.icon)"
                    classStyle="main-btn"
    ...
    methods: {
      ...
      _productDet (productId) {
        productDet({params: {productId}}).then(res => {
          let result = res.data
          this.product = result
          this.productMsg = result.content || ''
          // this.small = result.icon
          this.big = result.icon
        })
      },
      addCart (id, price, name, img) {
        if (!this.showMoveImg) {     // 动画是否在运动
          if (getStore('userId')) { // 登录了 直接存在用户名下
            let params = {
              params: {
                productId: id,
