        ...
        <div v-if="loginPage" class="content">
          <ul class="common-form">
            ...
            <li style="text-align: right" class="pr">
              <a href="http://120.79.198.111:8080/seger/shop/user/admin" target="_blank" style="padding: 0 5px">卖家登录</a>
      ...
      login () {
        ...
        if (!userName || !userPwd) {
          ...
          userLogin({userName, userPwd}).then(res => {
            if (res.code === 0) {
              setStore('userId', res.data)
              if (this.cart.length) {
                addCartBatch({productMsg: this.cart, userId: getStore('userId')}).then(res => {
                  if (res.code === 0) {
                    removeStore('buyCart')
                  }
  ...
  * {
  ...
  .v2 .dialog {
    ...
    .title {
      ...
      overflow: visible;
      position: relative;
      background-image: url(/static/images/stone.png);
